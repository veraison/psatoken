...
// SerializeStructToCBOR serializes the provided struct to CBOR using the
// provided EncMode. The behavior is broadly equivalent to calling
// em.Marashal(source), with the exception that embedded structs will be
// "merged" with their parents, so that the fields of the embedded struct will
// be treated as the fields of the parent when marshaling.
...
// PopulateStructFromCBOR deserializes provided data into dest (must be a
// pointer to a struct) using the provided DecMode.The behavior is broadly
// equivalent to calling dm.Unmarshal(data), with the exception that embedded
// structs' fields will be  treated as their parents' fields (rather than the
// default behavior of treating the embedded struct as a single field of its
// parent).
